...
describe ContractorsHelper do
  ...
  describe "#new_contractors_sentence" do
    ...
    context "There is one new contractor" do
      ...
      it { expect(helper.new_contractors_sentence(versions)).to eq "New contractor <em>Foo Company</em> added."}
    ...
    context "There are two new contractors" do
      ...
      it { expect(helper.new_contractors_sentence(versions)).to eq "New contractors <em>Foo Company</em> and <em>Bar Company</em> added."}
      it { expect(helper.new_contractors_sentence(versions)).to be_html_safe}
    end

    context "There are three new contractors" do
      let(:versions) {[version1, version2, version3]}
      it { expect(helper.new_contractors_sentence(versions)).to eq "New contractors <em>Foo Company</em>, <em>Bar Company</em>, and <em>Wiz Company</em> added."}
      it { expect(helper.new_contractors_sentence(versions)).to be_html_safe}
    end
  end
end
