...
describe ContractsHelper do
  ...
  describe '#new_contracts_sentence' do
    ...
    context 'there is one new contract' do
      ...
      it { expect(helper.new_contracts_sentence(versions)).to eq 'New contract: <a href="#123">Foo Service</a>.'}
    ...
    context 'there are two new contracts' do let(:versions) {[version1, version2]}
      it { expect(helper.new_contracts_sentence(versions)).to eq 'New contracts: <a href="#123">Foo Service</a> and <a href="#456">Bar Service</a>.'}
      it { expect(helper.new_contracts_sentence(versions)).to be_html_safe }
    end

    context 'there are three new contracts' do let(:versions) {[version1, version2, version3]}
      it { expect(helper.new_contracts_sentence(versions)).to eq 'New contracts: <a href="#123">Foo Service</a>, <a href="#456">Bar Service</a>, and <a href="#789">Wiz Service</a>.'}
      it { expect(helper.new_contracts_sentence(versions)).to be_html_safe }
    end
  end
end
